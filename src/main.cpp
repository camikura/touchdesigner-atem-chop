...
AtemCHOP::AtemCHOP(const OP_NodeInfo* info) { init(); }

AtemCHOP::~AtemCHOP() { stop(); }
...
void AtemCHOP::execute(CHOP_Output* output, const OP_Inputs* inputs,
  ...
  // auto reconnect
  if (int(now_t) - checkReconnTerm > int(last_send_packet_t) &&
      !isConnected()) {
    sendPacketStart();
  }
...
void AtemCHOP::getInfoCHOPChan(int32_t index, OP_InfoCHOPChan* chan,
  ...
  if (index >= 3) {
    chan->name->setString(
        topology_names[int(static_cast<__int64>(index) - 3)].c_str());
...
bool AtemCHOP::isConnecting() {
  return this->conn_state == connStateConnecting;
}
